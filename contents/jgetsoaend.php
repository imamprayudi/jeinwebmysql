...
$rs = $db->Execute("select * from soaend where (hd = 'H') and (suppcode = '" . $suppid . "') and (transdate = '" . $tgl . "')");
$ada = $rs->RecordCount();
if ($ada == 0)
{
  echo 'Data Nothing ....';
...
else
{
	...
	while (!$rs->EOF)
  {
	...
	echo '<a target="_blank" href="jsoaenddl.php?sid=' . $supp . '&tglid=' . $tgl . '">DOWNLOAD DATA TO CSV FORMAT</a>';
	echo '<br />';
	...
	echo '</tr>';
	$nomor = 0;
	$rs = $db->Execute("select * from soaend where (hd = 'D') and (suppcode = '" . $suppid . "') and (transdate = '" . $tgl . "') order by invoice, ok");
	while (!$rs->EOF)
  {
		$nomor++;
		echo '<tr>';
