...
<html>
<head>
<title>Get Forecast Yearly</title>
...
$rs = $db->Execute("select * from fcy where rt = 'H' and suppcode = '" . $suppid . "'");
$ada = $rs->RecordCount();
...
if ($ada == 0)
...
else
  {
    ...
    echo '<a target="_blank" href="jgetfcydl.php?sid=' . $supp . '">DOWNLOAD DATA TO CSV FORMAT</a>';
    echo '<br /><br />';
    ...
    $nomor = 0;
    $rs = $db->Execute("select * from fcy where rt = 'D' and suppcode = '" . $suppid . "' order by partno");
    while (!$rs->EOF)
     {
       $nomor++;
       echo '<tr>';
