<?php
  ...
  ****  
	...
	*/  
    ...
    $lanjut = TRUE;
    $hitung = 0;
    $fh = fopen('/var/www/html/uploads/pdi01webmid.txt','r');
    $line = fgets($fh);
    ...
    try{
      $rs = $db->Execute("insert into soamiddate(tanggal) select '{$transdate}'");      
      $rs->Close();
    }
    catch (exception $e)
    {   
	...
	while (($line = fgets($fh)) AND ($lanjut == TRUE)) {
      ...
      $sqlsoa = "insert into soamid(transdate,hd,tm,blnthn,suppcode,ok,tgl,po,posq,
