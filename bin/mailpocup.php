<?php
	...
	*/  
	...
	//	delete data
	$rs = $db->Execute("delete from mailpoctoday;");
	$rs->Close();	
	
	//	read text file
  $fh = fopen('/var/www/html/uploads/mail.poc.txt','r');
  while ($line = fgets($fh)) {
    set_time_limit(0);
    ...
    $oldqty = substr($line,197,10);
    $olddate = substr($line,209,10);
    $price = substr($line,221,10);
    $model = substr($line,233,20);
    $potype = substr($line,256,30);
    $rs = $db->Execute("insert into mailpoctoday(idno,rdate,rtime, supplier, 
      suppliername,actioncode,pono,partno,partname,newqty,newdate,oldqty,olddate,
      price,model,potype) select '{$idno}','{$rdate}','{$rtime}',
      '{$suppcode}','{$suppname}','{$actioncode}','{$pono}','{$partno}',
      '{$partname}','{$newqty}','{$newdate}','{$oldqty}','{$olddate}','{$price}','{$model}','{$potype}'");
		$rs->Close();
	}
	fclose($fh);
	echo 'Insert mailpoc from mail.poc.txt success<br />';
	//	connection close
	$db->Close();
?>
